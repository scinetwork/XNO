...
from ..layers.spectral_convolution_laplace import SpectralConvLaplace1D, SpectralConvLaplace2D, SpectralConvLaplace3D
from ..layers.padding import DomainPadding
from ..layers.lno_block import LNOBlocks
from ..layers.channel_mlp import ChannelMLP
from ..layers.complex import ComplexValued
from .base_model import BaseModel

class LNO(BaseModel, name='LNO'):
    """N-Dimensional Fourier Neural Operator. The LNO learns a mapping between
    spaces of functions discretized over regular grids using Fourier convolutions, 
    as described in [1]_.
    
    The key component of an LNO is its SpectralConv layer (see 
    ``neuralop.layers.spectral_convolution``), which is similar to a standard CNN 
    conv layer but operates in the frequency domain.

    For a deeper dive into the LNO architecture, refer to :ref:`LNO_intro`.

    Parameters
    ----------
    n_modes : Tuple[int]
        number of modes to keep in Fourier Layer, along each dimension
        The dimensionality of the LNO is inferred from ``len(n_modes)``
    in_channels : int
        Number of channels in input function
    out_channels : int
        Number of channels in output function
    hidden_channels : int
        width of the LNO (i.e. number of channels), by default 256
    ...
    lifting_channel_ratio : int, optional
        ...
        The number of liting channels in the lifting block of the LNO is
    ...
    projection_channel_ratio : int, optional
        ...
        The number of projection channels in the projection block of the LNO is
    ...
    positional_embedding : Union[str, nn.Module], optional
        ...
        before being passed through the LNO. Defaults to "grid"
    ...
    ~~ channel_mlp_dropout : float, optional
        dropout parameter for ChannelMLP in LNO Block, by default 0
    ~~ channel_mlp_expansion : float, optional
        expansion parameter for ChannelMLP in LNO Block, by default 0.5
    ...
    def get_blocks(self):
        return self.lno_blocks  # Return lno_blocks for LNO
    ...
    def forward(self, x, output_shape=None, **kwargs):
        ...
        for layer_idx in range(self.n_layers):
            x = self.lno_blocks(x, layer_idx, output_shape=output_shape[layer_idx])
    ...
    def n_modes(self, n_modes):
        self.lno_blocks.n_modes = n_modes
        self._n_modes = n_modes
...
TLNO = partialclass("TLNO", LNO)
TLNO1d = partialclass("TLNO1d", LNO1d)
TLNO2d = partialclass("TLNO2d", LNO2d)
TLNO3d = partialclass("TLNO3d", LNO3d)