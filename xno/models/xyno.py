...
from ..layers.xyno_block import XYNOBlocks
from ..layers.channel_mlp import ChannelMLP
from ..layers.complex import ComplexValued
from .base_model import BaseModel

class XYNO(BaseModel, name='XYNO'):
    """N-Dimensional Fourier Neural Operator. The XYNO learns a mapping between
    spaces of functions discretized over regular grids using different mathematical convolutions, 
    as described in ... .
    
    The key component of an XYNO is its SpectralConv layer (see 
    ...
    n_modes : Tuple[int]
        ...
        The dimensionality of the XYNO is inferred from ``len(n_modes)``
    in_channels : int
        Number of channels in input function
    out_channels : int
        Number of channels in output function
    hidden_channels : int
        width of the XYNO (i.e. number of channels), by default 256
    ...
    lifting_channel_ratio : int, optional
        ...
        The number of liting channels in the lifting block of the XYNO is
    ...
    projection_channel_ratio : int, optional
        ...
        The number of projection channels in the projection block of the XYNO is
    ...
    positional_embedding : Union[str, nn.Module], optional
        ...
        before being passed through the XYNO. Defaults to "grid"
    ...
    channel_mlp_dropout : float, optional
        dropout parameter for ChannelMLP in XYNO Block, by default 0
    channel_mlp_expansion : float, optional
        expansion parameter for ChannelMLP in XYNO Block, by default 0.5
    ...
    xno_skip : str {'linear', 'identity', 'soft-gating'}, optional
        Type of skip connection to use in XYNO layers, by default 'linear'
    ...
    stabilizer : str {'tanh'} | None, optional
        whether to use a tanh stabilizer in XYNO block, by default None
    ...
    factorization : str, optional
        Tensor factorization of the XYNO layer weights to use, by default None.
    ...
    preactivation : bool, optional (**DEACTIVATED**)
        whether to compute XYNO forward pass with resnet-style preactivation, by default False
    conv_module : nn.Module, optional
        module to use for XYNOBlock's convolutions, by default None
    
    Examples
    ---------
    
    >>> from neuralop.models import XYNO
    >>> model = XYNO(n_modes=(12,12), in_channels=1, out_channels=1, hidden_channels=64, transformation="FNO")
    >>> model
    XYNO(
    (positional_embedding): GridEmbeddingND()
    (xno_blocks): XYNOBlocks(
        (convs): SpectralConv(
        (weight): ModuleList(
    ...
    def __init__(
        ...
        **kwargs
        ...
        # When updated, change should be reflected in XYNO blocks
        ...
        if positional_embedding == "grid":
            ...
            raise ValueError(f"Error: tried to instantiate XYNO positional embedding with {positional_embedding},\
    ...
    def get_blocks(self):
        return self.xno_blocks  # Return fno_blocks for XYNO
    ...
    def forward(self, x, output_shape=None, **kwargs):
        ...
        4. Applies `n_layers` Fourier(or any other kernels)/XYNO layers in sequence (SpectralConvolution + skip connections, nonlinearity) 
        ...
        output_shape : {tuple, tuple list, None}, default is None
            ...
            * If tuple, specifies the output-shape of the **last** XYNO Block
...
class XYNO1d(XYNO):
    ...
    For the full list of parameters, see :class:`neuralop.models.XYNO`.
...
TXYNO = partialclass("TXYNO", XYNO, factorization="Tucker")
TXYNO1d = partialclass("TXYNO1d", XYNO1d, factorization="Tucker")
TXYNO2d = partialclass("TXYNO2d", XYNO2d, factorization="Tucker")
TXYNO3d = partialclass("TXYNO3d", XYNO3d, factorization="Tucker")
