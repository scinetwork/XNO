...
class IncrementalFNOTrainer(Trainer):
    ...
    def loss_gap(self, loss):
        ...
        self.ndim = len(self.model.xno_blocks.convs[0].n_modes)

        # method 1: loss_gap
        incremental_modes = self.model.xno_blocks.convs[0].n_modes[0]
        max_modes = self.model.xno_blocks.convs[0].max_n_modes[0]
        ...
        self.model.xno_blocks.convs[0].n_modes = modes_list
    ...
    def grad_explained(self):
        ...
        if not hasattr(self, 'accumulated_grad'):
            self.accumulated_grad = torch.zeros_like(
                self.model.xno_blocks.convs[0].weight)
        ...
        if self.grad_iter <= self.incremental_grad_max_iter:
            self.grad_iter += 1
            self.accumulated_grad += self.model.xno_blocks.convs[0].weight
        else:
            incremental_final = []
            for i in range(self.ndim):
                max_modes = self.model.xno_blocks.convs[i].max_n_modes[0]
                incremental_modes = self.model.xno_blocks.convs[0].n_modes[0]
            ...
            self.model.xno_blocks.convs[0].n_modes = tuple(modes_list)
