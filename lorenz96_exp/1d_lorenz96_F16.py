...
data_path = 'data/lorenz96_dataset_F16.mat'
data_name = '1d_lorenz96_F16'
time_step_mode = "s"
...
output_file = open(f"save/{data_name}/{data_name}_{transformation.lower()}.txt", "w")
...
x = X[shift_k:shift_k+res_params['total'], :]    
x_p1 = X[1+shift_k:1+shift_k+res_params['total'],:]  
y = x_p1
if target_value == "delta":
    y = x_p1 - x # Target value represents the change (delta) to the next step

x_train = x[:res_params['ntrain'], :] 
y_train = y[:res_params['ntrain'], :]
x_test = x[res_params['ntrain']:res_params['ntrain']+res_params['ntest'], :]
y_test = y[res_params['ntrain']:res_params['ntrain']+res_params['ntest'], :]
...
train_loader = DataLoader(train_loader, batch_size=train_batch, shuffle=True)
test_loader = DataLoader(test_loader, batch_size=test_batch, shuffle=True)
test_loader = {
